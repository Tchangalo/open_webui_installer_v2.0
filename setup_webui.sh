...
main() {
  ...
  if [ "${ACTIONS_PERFORMED}" -gt 0 ]; then
      ...
      # Reboot only if at least one container was actually started.
      # This prevents reboot when only docker/docker-compose were installed without starting containers,
      # or when only removals were performed.
      if [ "${CONTAINERS_STARTED}" -gt 0 ]; then
        info "Containers started (${CONTAINERS_STARTED}) — rebooting..."
        ${SUDO} reboot
      else
        info "No containers started — reboot skipped."
      fi
  else
      info "No actions selected. Nothing to do."
  fi
